...
function App() {
  ...
  // Quiet reading counts the verses that stay on screen. Listening still marks
  // the chapter when the last verse is heard.
  useEffect(() => {
    if (speechState !== 'idle' || chapterLoading || !chapter || view !== 'reader') return;
    ...
    const io = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        ...
        if (!verse) continue;
        const timer = pending.get(verse);
        if (entry.isIntersecting && entry.intersectionRatio >= 0.6) {
          if (timer != null || ready.has(verse)) continue;
          pending.set(verse, window.setTimeout(() => { pending.delete(verse); ready.add(verse); }, 1500));
        } else if (timer != null) {
          window.clearTimeout(timer);
          pending.delete(verse);
        }
      }
    }, { threshold: 0.6 });
    for (const element of Object.values(verseRefs.current)) if (element) io.observe(element);
    const flush = window.setInterval(() => {
      if (!ready.size) return;
      const verses = [...ready];
      ready.clear();
      void recordChapterReadInGroup(app.bookId, chapterNumber, verses);
    }, 2000);
    return () => {
      io.disconnect();
  ...
  }, [speechState, chapterLoading, chapter, view, app.bookId, chapterNumber, recordChapterReadInGroup]);
  ...
  const speechControls = <>
    ...
    {speechState==='idle' && chapter && <button onClick={()=>{ void recordChapterReadInGroup(app.bookId, chapterNumber, chapter.verses.map((verse)=>verse.ref.verse)); }} disabled={ledger.chapterDone(app.bookId, chapterNumber)}>{ledger.chapterDone(app.bookId, chapterNumber) ? label.chapterMarkedRead : label.markChapterRead}</button>}
    ...
    {(speechState==='paused' || (speechState==='idle' && app.resumeAt?.bookId===app.bookId && app.resumeAt.chapter===chapterNumber)) && <button onClick={app.listenFromBeginning} disabled={!chapter}>{label.listenFromBeginning}</button>}
    {speechState!=='idle' && <button onClick={()=>app.stopSpeech({remember:true})}>{label.stop}</button>}
  </>;
  ...
  const group = <div className="group-content">
    {!party.active ? <><span className="workspace-eyebrow">Read. Reflect. Together.</span><h2>A seat at the table.</h2><p>Share a passage, talk face to face, and follow the host’s reading.</p>
      ...
      <div className="group-entry-actions"><form className="join-form" onSubmit={e=>{e.preventDefault();speech.unlock?.();unlockRemoteAudio();party.joinParty(partyCode);}}><h3>Join a group</h3><label>Room code<input autoCapitalize="none" autoCorrect="off" spellCheck={false} aria-label="Room code" value={partyCode} onChange={e=>setPartyCode(e.target.value)} placeholder="Enter code"/></label><button disabled={!partyCode.trim()}>Join group</button></form><section className="group-start"><h3>Start a group</h3><label className="check-label"><input type="checkbox" checked={createFindable} onChange={e=>setCreateFindable(e.target.checked)}/>List this group publicly</label>
      ...
      <button className="primary" onClick={()=>{speech.unlock?.();unlockRemoteAudio();party.createParty({findable:createFindable});}}>Start a group</button></section>
      </div>
    ...
    {party.error && <p role="alert">{party.error==='session-replaced'?'This group session moved to another tab or refresh. Leave this old session to continue.':`Could not connect: ${party.error}. Leave the room and try joining again.`}</p>}
  </div>;
  ...
  <div hidden={view!=='reader'} className={`study-workspace ${tools.focused?'has-tools':''} ${party.active?'group-reading':''} ${library?'has-library':''} ${tools.focused&&(pageTools.includes(tools.focused)||(!party.active&&tools.focused==='group'))?'page-tool':''}`} data-mobile-view={mobileView}>
    <header className="workspace-header"><button className="workspace-brand" onClick={openHome} aria-label={label.home}><BookBibleIcon/><span>The Word</span></button>
      ...
      <nav aria-label="Reader tools">
        ...
        <button disabled={party.active} aria-pressed={tools.focused==='progress'||tools.focused==='study'} onClick={()=>openTool('progress')}><ReaderIcon name="award"/><span>{label.progress}</span></button>
        <button aria-pressed={tools.focused==='group'} onClick={()=>openTool('group')}><ReaderIcon name="people"/><span>{ui.group}</span></button>
        <button disabled={party.active} aria-label={label.settings} aria-pressed={tools.focused==='settings'} onClick={()=>openTool('settings')}><ReaderIcon name="settings"/><span>{label.settings}</span></button>
      </nav>
    </header>
    ...
    <main className="workspace-body">
      ...
      <section className="reader-column" aria-label="Scripture passage">
        ...
        {chapterLoading?<div className="empty-state" role="status">{label.loading}</div>:chapter?<article className="reader" style={{fontSize:`${app.fontSize}px`}}>{chapter.verses.map(verse=>{
          ...
          return <span key={number} data-verse={number} ref={el=>{verseRefs.current[number]=el;}} className={['verse',selected?'selected':'',speaking?'speaking':'',shared?'shared-highlight':'',following?'following':'',app.focusedVerse===number?'focused':''].filter(Boolean).join(' ')}>
